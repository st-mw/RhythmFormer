...
class RhythmFormerTrainer(BaseTrainer):
    ...
    def extract(self, data_loader):
        ...
        predictions_df = pd.DataFrame(columns=['filename', 'i', 'prediction'])

        with torch.no_grad():
            for index, extract_batch in enumerate(data_loader['extract']):

                # extract_batch[0] is the first element of the tuple returned 
                # by __getitem__ in the dataloader. It is the data.
                ...
                for ib in range(batch_size):
                    ...
                    predictions_dict[clip_filename] = clip_predictions_array
            ...
            preds_by_vid_df.columns = ['filename'] + [f'{i}' for i in range(0, 160)]
            preds_by_vid_df.to_csv('predictions_by_vid.csv', index=False)
            
            predictions_df = pd.DataFrame(predictions_list)
            predictions_df.to_csv('predictions.csv', index=False)
            print("rPPG saved to predictions_by_vid.csv and predictions.csv")
